...
time_t setTimeSource(void) {
	...
	breakTime(now(), tm);
	if(debug) snooze_usb.printf("Time:    Date/Time at startup: %02d.%02d.%d, %02dh%02dm%02ds\n", tm.Day, tm.Month, (tm.Year+1970) ,tm.Hour, tm.Minute, tm.Second);
...
void setCurTime(time_t cur_time, enum tSources source) {
	...
	switch(source) {
		case TSOURCE_GPS:
            ...
            if(debug) snooze_usb.printf("Time:    Current time set to: %02dh%02dm%02ds\n", tm.Hour, tm.Minute, tm.Second);
	...
	// if(debug) snooze_usb.printf("Time:    Time adjusted from source#%d. Current time: %ld\n", source, now());
}
...
void setWaitAlarm(void) {
    ...
    if(next_time > now()) {
        ...
        if(debug) snooze_usb.printf("Time:    Mismatching time. Stopping recording\n");
...
void alarmAdvTimeout(void) {
    if(debug) snooze_usb.printf("Time:    Advertising timeout.\n");
...
void timerRecDone(void) {
	...
	if(debug) snooze_usb.printf("Time:    Recording#%d done.", (next_record.cnt+1));

	if((rec_window.occurences == 0) || (next_record.cnt < (rec_window.occurences-1))) {
		if(debug) snooze_usb.println(" Going on...");
		working_state.rec_state = RECSTATE_REQ_PAUSE;
	}
	else {
		if(debug) snooze_usb.println(" Finished!");
		working_state.rec_state = RECSTATE_REQ_OFF;
	}
}
...
void alarmNextRec(void) {
	removeWaitAlarm();
	if(debug) snooze_usb.printf("Time:    Starting recording#%d\n", (next_record.cnt+1));
