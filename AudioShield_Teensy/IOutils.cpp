...
void toggleCb(struct leds_s *ld) {
	// if(debug) snooze_usb.print("I/O:     toggleCb called with led#"); if(debug) snooze_usb.println(ld->pin);
	// if(debug) snooze_usb.print("I/O:     address: 0x"); if(debug) snooze_usb.println((unsigned long)toggleCb, HEX);
	switch(ld->mode) {
		case LED_MODE_OFF:
		...
		// if(debug) snooze_usb.println("I/O:     Waiting mode: regular medium toggle");
		ld->status = !ld->status;
		break;
		...
		// if(debug) snooze_usb.print("I/O:     Warning mode: "); if(debug) snooze_usb.print(ld->cnt); if(debug) snooze_usb.println(" toggles");
		if(ld->cnt++ > 12) {
			// stopLED(ld);
		...
		// if(debug) snooze_usb.println("I/O:     Fast flashing mode: irregular flash toggle");
		if(ld->status == LED_ON) {
		...
		// if(debug) snooze_usb.println("I/O:     Slow flashing mode: irregular flash toggle");
		if(ld->status == LED_ON) {
			ld->timer.update(LED_BLINK_FAST_MS);
		}
		else {
...
void startLED(struct leds_s *ld, enum lMode mode) {
	// if(debug) snooze_usb.print("I/O:     startLED: LED#"); if(debug) snooze_usb.print(ld->pin);
	// if(debug) snooze_usb.print(", mode = "); if(debug) snooze_usb.println(mode);
	...
	switch(mode) {
		...
		// if(debug) snooze_usb.print("I/O:     Timer started, returning at 0x"); if(debug) snooze_usb.println((unsigned long)ld->toggle, HEX);
		break;
...
void stopLED(struct leds_s *ld) {
    // if(debug) snooze_usb.print("I/O:     StopLED: LED#"); if(debug) snooze_usb.println(ld->pin);
	ld->timer.end();
