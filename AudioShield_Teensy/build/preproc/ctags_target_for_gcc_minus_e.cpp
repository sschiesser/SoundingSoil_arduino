# 1 "C:\\_code\\Arduino\\SoundingSoil\\AudioShield_Teensy\\AudioShield_Teensy.ino"
# 1 "C:\\_code\\Arduino\\SoundingSoil\\AudioShield_Teensy\\AudioShield_Teensy.ino"
/*
...
*/
# 8 "C:\\_code\\Arduino\\SoundingSoil\\AudioShield_Teensy\\AudioShield_Teensy.ino"
# 9 "C:\\_code\\Arduino\\SoundingSoil\\AudioShield_Teensy\\AudioShield_Teensy.ino" 2
...
void setup() {
 ...
 initLEDButtons();
 button_wakeup.pinMode(2 /*2						2*/, 2, 2);
 button_wakeup.pinMode(16 /*4						16*/, 2, 2);
  button_wakeup.pinMode(21 /*21					21*/, 2, 2);
...
SLEEP:
 ...
 (*(volatile uint32_t *)0x4004803C) /* System Clock Gating Control Register 6*/ &= ~((uint32_t)0x00008000) /* I2S Clock Gate Control*/;
 Alarm.delay(100);
 ...
 if(who == 35) {
  ...
  Bounce cur_bt = Bounce(who, 20);
  elapsedMillis timeout = 0;
  while (timeout < (20 +1)) cur_bt.update();
...
WORK:
 ...
 if(but_rec.fallingEdge()) button_call = (enum bCalls)2 /*2						2*/;
 if(but_mon.fallingEdge()) button_call = (enum bCalls)16 /*4						16*/;
 if(but_blue.fallingEdge()) button_call = (enum bCalls)21 /*21					21*/;

  // centralized button call actions coming from SLEEP or WORK mode
  if(button_call == 2 /*2						2*/) {
 ...
 switch(working_state.mon_state) {
  case MONSTATE_REQ_ON: {
   Serial.printf("MON req... states: BT %d, BLE %d, REC %d, MON %d\n",
 ...
 switch(working_state.ble_state) {
  case BLESTATE_REQ_ADV: {
   if(working_state.bt_state == BTSTATE_CONNECTED) {
    ...
    alarm_adv_id = Alarm.timerOnce(3600, alarmAdvTimeout);
   }
   bc127AdvStart();
 ...
 // Serial messaging
  ...
  Serial.printf("Sent to BLUEPORT: %s\n", manInput.c_str());
  }
 ...
 */
# 418 "C:\\_code\\Arduino\\SoundingSoil\\AudioShield_Teensy\\AudioShield_Teensy.ino"
void setDefaultValues(void) {
  working_state.rec_state = RECSTATE_OFF;
  working_state.mon_state = MONSTATE_OFF;
  working_state.bt_state = BTSTATE_OFF;
  working_state.ble_state = BLESTATE_OFF;
 rec_window.length.Second = 0 /* }*/;
 rec_window.length.Minute = 0 /* } Zero values -> continuous recording*/;
 rec_window.length.Hour = 0 /* }*/;
 rec_window.length.Day = 1;
 rec_window.length.Month = 1;
 rec_window.length.Year = 0;
 rec_window.period.Second = 20;
 rec_window.period.Minute = 0;
 rec_window.period.Hour = 0;
 rec_window.period.Day = 1;
 rec_window.period.Month = 1;
 rec_window.period.Year = 0;
 rec_window.occurences = 3 /* Zero value -> infinite repetitions*/;
 last_record.cnt = 0;
